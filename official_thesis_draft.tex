...
% Research problem
However, deploying VLA policies into real-world, human-centric environments intro-
duces severe safety risks. While hallucinations in pure language models merely yield incor-
rect text, ungrounded actions or spatial errors in VLAs directly cause physical collisions,
property damage, or catastrophic injury.

% Research aims and/or objectives

The aim of this work is to determine whether the safety gains provided by external inference-time mechanisms can be in turn internalized in the model weights, and thus making the VLA inherently safer. 
...
\begin{enumerate}[label=\textbf{RQ\arabic*.}, leftmargin=3.2em]
  ...
  \item What determines whether the transfer occurs --- does the source of the demonstrations
        matter, and what mechanism explains the difference?
        (Sections~\ref{sec:res:source} and~\ref{sec:res:mechanism})
...
Chapter~\ref{ch:background} reviews the literature in four strands: runtime safety filtering for
learned policies, learned safety representations that replace the analytic barrier, work on what
makes corrective demonstrations transferable, and policy-level alternatives that pursue safety
through constrained learning rather than filtering. It closes by locating the gap this study
...
Chapter~\ref{ch:background}, sets out the limitations of the study, and identifies the work that
would follow from it. Chapter~\ref{ch:conclusion} returns to the research questions and states what
the study answers, what it bounds, and what remains open. Supporting material that would interrupt
